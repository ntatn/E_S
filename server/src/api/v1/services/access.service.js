...
class AccessService {
    ...
    static handleRefreshToken = async (refreshToken) => {
        ...
        if(!foundUser) throw new AuthFailureError(`User not registered 2 ${foundUser}`)
    ...
    */ 
    static login = async ( { email, password, refreshToken = null } ) => {
        ...
        return {user: getIntoData({fileds: ['_id', 'name', 'status'], object: foundUser}), token: tokens_}
    }   
    ...
    static signUp = async ({name, email, password, level}) => {
        try{
            ...
            const newUser = await user.create({
                name, email, password: passwordHash, level
            })
            if(newUser){
                const publicKey = crypto.randomBytes(64).toString('hex')
                const privateKey = crypto.randomBytes(64).toString('hex')
                ...
                return  new CREATED({
                        metadata: {
                            user: getIntoData({fileds: ['_id', 'name', 'email', 'level'], object: newUser}),
                            tokens
                        }
                    })     
            }
